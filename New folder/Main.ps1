...
function Find-LspFolder {
    ...
    foreach ($root in $Roots) {
        ...
        # 2) Försök med -Recurse + -Filter (snabbare än full enumerering)
        foreach ($f in $filters) {
            try {
...
$btnScan.Add_Click({
    ...
    Gui-Log "🔎 Söker filer för $lsp…" -Immediate
...
if (-not $folder) {
    ...
    if ($env:IPT_ROOT) { Gui-Log "ℹ️ IPT_ROOT=$($env:IPT_ROOT)" 'Info' }
    $rootInfo = $RootPaths | ForEach-Object { "{0} ({1})" -f $_, $(if (Test-Path -LiteralPath $_) { "OK" } else { "MISSING" }) }
    Gui-Log ("ℹ️ Sökvägar: " + ($rootInfo -join " | ")) 'Info'
    return
}
...
# ----------------------------
# Robust config-flag helper
# ----------------------------
if (-not (Get-Command Test-ConfigFlag -ErrorAction SilentlyContinue)) {
    function Test-ConfigFlag {
        ...
        if ($Config -is [hashtable]) {
            if ($Config.ContainsKey($Name)) { return [bool]$Config[$Name] }
            return $Default
        }

        # 2) IDictionary (t.ex. OrderedDictionary)
        if ($Config -is [System.Collections.IDictionary]) {
            # Vissa IDictionary har Contains() men inte ContainsKey()
            $hasContains = $Config.GetType().GetMethod('Contains') -ne $null
            if ($hasContains) {
                try {
                    if ($Config.Contains($Name)) { return [bool]$Config[$Name] }
                } catch {
                    # Ignorera och försök nyckelloop nedan
                }
            }
            foreach ($k in $Config.Keys) {
                if (("$k") -eq $Name) { return [bool]$Config[$k] }
            }
            return $Default
        }

        # 3) PSCustomObject / andra PSObjects
        try {
            $prop = $Config.PSObject.Properties[$Name]
            if ($prop) { return [bool]$prop.Value }
        } catch {
            # Ignorera och fall tillbaka på Default
        }
...
$btnBuild.Add_Click({
    ...
    try {
        ...
        # ============================
        try {
            if ((Test-ConfigFlag $Config 'EnableRuleEngine' $false) -and
                ...
                if ($csvRows -and $csvRows.Count -gt 0) {
                    $csvObjs = @($csvRows)
                    Gui-Log ("🧠 RuleEngine (shadow): CSV-källa: Import-CsvRows ({0})" -f $csvObjs.Count) 'Info'
                } else {
                    try {
                    ...
                    Gui-Log ("🧠 RuleEngine (shadow): CSV-källa: Fallback-raw ({0})" -f ($csvObjs.Count)) 'Info'
                }

                $script:RuleEngineCsvObjs = $csvObjs

                if (-not $csvObjs -or $csvObjs.Count -eq 0) {
                    Gui-Log "⚠️ RuleEngine (shadow): CSV-objekt saknas (0 rader) – hoppar över." 'Warn'
                    $script:RuleEngineShadow = $null
                } else {
        ...
        if ($totalRows -eq 0) {
            ...
            if ($wsOut2.Dimension) { try { $wsOut2.Cells[$wsOut2.Dimension.Address].AutoFitColumns() | Out-Null } catch {} }
        }
    ...
    try {
        if ($selLsp -and (Test-Path -LiteralPath $selLsp)) {
            try {
                ...
                # ==============================
                try {
                    if ($Config -and $Config.Contains('EnableEquipmentScan') -and -not $Config.EnableEquipmentScan) {
                        # Fast path: do not scan worksheet tables (slow). Use CSV Instrument S/N list instead (no due dates).
                        $instObjs = @()
                        try {
                            foreach ($s in (@($csvInstrumentSerials) | Select-Object -First 12)) {
                                $instObjs += [pscustomobject]@{ Id = $s; CalibrationDueDate = 'N/A' }
                            }
                        } catch {}
                        $eqInfo = [pscustomobject]@{ WorksheetName = 'CSV (Instrument S/N)'; Pipettes = @(); Instruments = $instObjs }
                        Gui-Log 'ℹ️ Equipment-scan avstängd (snabbt läge). Instrument tas från CSV.' 'Info'
                    } else {
...
try {
    if (Test-Path -LiteralPath $UtrustningListPath) {
        ...
        Gui-Log ("ℹ️ Infinity/GX mall saknas: $UtrustningListPath") 'Info'
    }
} catch {
...
# ============================
try {
    # FIX: om checkboxen inte finns → default är att köra SharePoint Info (dvs "som vanligt")
    $skipSpInfo = $false
    try {
        if ($chkSharePointInfo -and $chkSharePointInfo.Checked -eq $false) { $skipSpInfo = $true }
    } catch {}
...
try {
    ...
    Gui-Log "⚠️ Kunde inte sätta tab-färg: $($_.Exception.Message)" 'Warn'
        ...
        try {
            ...
            try {
                if ((Test-ConfigFlag $Config 'EnableRuleEngine' $false) -and
                    (Test-ConfigFlag $Config 'EnableRuleEngineDebugSheet' $false) -and
                    $selCsv -and (Test-Path -LiteralPath $selCsv)) {
                    ...
                    if (-not $script:RuleEngineShadow -or -not $script:RuleEngineShadow.Rows -or $script:RuleEngineShadow.Rows.Count -eq 0) {
                        ...
                        Gui-Log "🧠 RuleEngine (shadow): saknas vid Save → bygger nu..." 'Warn'

                        $rb2 = $script:RuleBankCache
                        if (-not $rb2) { $rb2 = Load-RuleBank -RuleBankDir $Config.RuleBankDir }
                        ...
                        if ($csvObjs2 -and $csvObjs2.Count -gt 0) {
                            ...
                            Gui-Log "⚠️ RuleEngine (shadow): kunde inte bygga vid Save (0 rader)." 'Warn'
                        }
                    }

                    if ($script:RuleEngineShadow -and $script:RuleEngineShadow.Rows -and $script:RuleEngineShadow.Rows.Count -gt 0) {
                        Gui-Log "🧠 Skriver RuleEngine_Debug (shadow)..." 'Info'
                        ...
                        Gui-Log "⚠️ RuleEngine_Debug hoppar över: inget RuleEngine-resultat." 'Warn'
                    }
                }
            } catch {
