...
<#
  RuleBank.ps1
  - DATA ONLY: one root object `$RuleBank (global) containing all assay profiles.
  - Generated from real 'Tests Summary' exports + ErrorBank.xlsx.
  - X/+ parity logic intentionally skipped for now (future majority/parity rule).
#>
  ...
  'AssayProfiles' =   [ordered]@{
    'Respiratory Panel IUO' =     [ordered]@{
      ...
      'ExpectedLiteralByRole' =       [ordered]@{
        'NEG_ALL' = 'Adenovirus NEGATIVE;Coronavirus NEGATIVE;SARS-CoV-2 NEGATIVE;Flu A NEGATIVE;Flu B NEGATIVE;hMPV NEGATIVE;Parainfluenza NEGATIVE;Rhino-Enterovirus NEGATIVE;RSV NEGATIVE;B. parapertussis NEGATIVE;B. pertussis NEGATIVE;Chlam. pneumoniae NEGATIVE;Myco. pneumoniae NEGATIVE'
    ...
    'Xpert CT_NG' =     [ordered]@{
      ...
      'ExpectedResultByRole' =       [ordered]@{
        'NEG' =         [ordered]@{
          'WhenStatusIn' = @('Done')
          'Severity' = 'Error'
          'ResultRegex' = @('(?i)^CT\s+NOT\s+DETECTED;NG\s+NOT\s+DETECTED$', '(?i)^INVALID$')
        }
        'POS' =         [ordered]@{
          'WhenStatusIn' = @('Done')
          'Severity' = 'Error'
          'ResultRegex' = @('(?i)^CT\s+DETECTED;NG\s+DETECTED$')
        }
      }
    }
    ...
    'Xpert MTB-RIF Assay G4' =     [ordered]@{
      ...
      'ExpectedResultByRole' =       [ordered]@{
        'NEG' =         [ordered]@{
          'WhenStatusIn' = @('Done')
          'Severity' = 'Error'
          'ResultRegex' = @('(?i)^MTB\s+NOT\s+DETECTED$', '(?i)^ERROR$')
        }
        'NEG_2' =         [ordered]@{
          'WhenStatusIn' = @('Done')
          'Severity' = 'Error'
          'ResultRegex' = @('(?i)^MTB\s+DETECTED\s+MEDIUM;Rif\s+Resistance\s+NOT\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+LOW;Rif\s+Resistance\s+NOT\s+DETECTED$')
        }
      }
    }
    ...
    'Xpert MTB-RIF Ultra' =     [ordered]@{
      ...
      'ExpectedResultByTestType' =       [ordered]@{
        ...
        'Positive Control 1' =         [ordered]@{
          'WhenStatusIn' = @('Done')
          'Severity' = 'Error'
          'ResultRegex' = @('(?i)^MTB\s+DETECTED\s+LOW;\s*RIF\s+Resistance\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+VERY\s+LOW;\s*RIF\s+Resistance\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+MEDIUM;\s*RIF\s+Resistance\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+HIGH;\s*RIF\s+Resistance\s+DETECTED$')
        }
        'Positive Control 2' =         [ordered]@{
          'WhenStatusIn' = @('Done')
          'Severity' = 'Error'
          'ResultRegex' = @('(?i)^MTB\s+DETECTED\s+LOW;\s*RIF\s+Resistance\s+NOT\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+VERY\s+LOW;\s*RIF\s+Resistance\s+NOT\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+MEDIUM;\s*RIF\s+Resistance\s+NOT\s+DETECTED$', '(?i)^MTB\s+DETECTED\s+HIGH;\s*RIF\s+Resistance\s+NOT\s+DETECTED$')
        }
      }
    }
