...
# Apply basic styling to header row to improve readability
try {
    ...
    $headerRange.Style.Font.Color.SetColor([System.Drawing.Color]::FromArgb(0,0,0))
} catch {}

# Turn the data range into an Excel table for banded rows and filter drop-downs
try {
    $tableStart = $tableHeaderRow
    $tableEnd = $tableHeaderRow + 1
    $tblRange = $ws.Cells[$tableHeaderRow,1,$tableHeaderRow,$headers.Count]
    $tblRangeData = $ws.Cells[$tableHeaderRow,1,$tableHeaderRow,$headers.Count]
} catch {}
