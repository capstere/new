...
$Colors = @{
    ...
    # Tabell
    TableHeaderBg  = [System.Drawing.Color]::FromArgb(91, 155, 213)   # Blå
...
# ============================================================================
$row = 1
$ws.Cells.Item($row, 1).Value = '📊 CSV Sammanfattning - Regelmotor-analys'
...
if ($sum -and $sum.InstrumentError -ne $null -and $sum.InstrumentError -gt 0) {
    Write-KV -Row $row -Key 'Instrument Error' -Value $sum.InstrumentError -Warning
    $row++
}

# Delam och ersättningar
if ($sum -and $sum.DelamCount -ne $null -and $sum.DelamCount -gt 0) { 
    Write-KV -Row $row -Key 'Delamineringar' -Value $sum.DelamCount -Warning
    $row++ 
}
if ($sum -and $sum.ReplacementCount -ne $null -and $sum.ReplacementCount -gt 0) { 
    Write-KV -Row $row -Key 'Ersättningar (A/AA/AAA)' -Value $sum.ReplacementCount -Warning
    $row++ 
}

# Omkörning
if ($sum.RetestYes -gt 0) {
    Write-KV -Row $row -Key 'Behöver omkörning (YES)' -Value $sum.RetestYes -Warning
    $row++
}
...
if ($qc) {
    if ($qc.DuplicateSampleIdCount -gt 0) {
        Write-KV -Row $row -Key 'Dubbletter Sample ID' -Value $qc.DuplicateSampleIdCount -Warning
        $row++
    }
    if ($qc.DuplicateCartridgeSnCount -gt 0) {
        Write-KV -Row $row -Key 'Dubbletter Cartridge S/N' -Value $qc.DuplicateCartridgeSnCount -Warning
        $row++
    }
    if ($qc.HotModuleCount -gt 0) {
        Write-KV -Row $row -Key 'Moduler med ≥3 fel' -Value $qc.HotModuleCount -Warning
        $row++
    }
}
...
if ($pressureGE90 -gt 0) {
    Write-KV -Row $row -Key 'Max Pressure ≥ 90 PSI' -Value $pressureGE90 -Minor
    $row++
}
...
if ($pressureFailNoError -gt 0) {
    Write-KV -Row $row -Key 'Max Pressure Failure (utan Error Code)' -Value $pressureFailNoError -Minor
    $row++
}
...
for ($i = 0; $i -lt $rowCount; $i++) {
    ...
    if ($rawDev -eq 'ERROR') {
        ...
        $isRealError = $false
        if ($errCode) {
            $codeUC = $errCode.ToUpperInvariant()
            if ($codeUC -ne 'OTHER') { $isRealError = $true }
        }
        if ($isRealError) {
...
for ($i = 0; $i -lt $rowCount; $i++) {
    ...
    switch ($dev) {
        'FP' {
            ...
            # Lägg till "Major Functional" i Error Code om tom
            $existingCode = ($data[$i, 1] + '').Trim()
            if (-not $existingCode) {
                $ws.Cells.Item($dataRow, 2).Value = 'Major Functional'
            }
...
try {
    ...
    $ws.Column(18).Width = [Math]::Max($ws.Column(18).Width, 30) # Error (kan vara lång)
} catch {}
