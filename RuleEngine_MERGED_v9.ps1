...
foreach ($r in $results) {
    ...
    if ((($r.Deviation + '')).Trim().ToUpperInvariant() -eq 'ERROR') {
        ...
        # Minor Functional om felkod finns (ej tom) eller MTB indeterminate
        if ( ($code -and $code.Length -gt 0) -or $isMtbInd ) {
...
for ($i = 0; $i -lt $rowCount; $i++) {
    ...
    switch ($rawDev) {
        'ERROR' {
            # Om vi har en felkod (oavsett om den finns i regelbanken) eller MTB indeterminate
            # så räknas detta som Minor Functional. Saknas felkod helt behandlas raden som Instrument Error.
            if ( ($errCode -and $errCode.Trim().Length -gt 0) -or $isMtbInd ) {
