...
# ------------------------------------------------------------
# Human-friendly display names ("how we talk")
# - Used by Information2 + future writers to show short readable names.
# - Keep keys as CANONICAL assay names.
# ------------------------------------------------------------
$assayDisplayNames = [ordered]@{
  # Examples (edit locally):
  # 'Xpert Xpress SARS-CoV-2 CE-IVD' = 'Xpress SARS-CoV-2 (CE-IVD)'
}

# Internal keys -> display labels (used when showing missing columns etc.)
$columnDisplayNames = [ordered]@{
  'Assay'       = 'Assay'
  'AssayVer'    = 'Assay Version'
  'ReagentLot'  = 'Reagent Lot ID (LSP)'
  'SampleId'    = 'Sample ID'
  'CartridgeSn' = 'Cartridge S/N'
  'ModuleSn'    = 'Module S/N'
  'TestType'    = 'Test Type'
  'Status'      = 'Status'
...
$global:RuleBank = [ordered]@{
  ...
  AssayDisplayNames  = $assayDisplayNames
