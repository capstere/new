...
# NOTE (v06.2): CSV bundle + delimiter detection moved to Modules/CsvBundle.ps1.
# NOTE (v06.2): EPPlus loader moved to Modules/EpplusLoader.ps1 (single source of truth).
# DataHelpers.ps1 contains formatting helpers + worksheet/seal-test parsing helpers.
...
function Get-AssayFromCsv {
    ...
    # Prefer CsvBundle (read-once + robust delimiter/header detection)
    try {
        if (Get-Command Get-TestsSummaryBundle -ErrorAction SilentlyContinue) {
            $b = Get-TestsSummaryBundle -Path $Path
            if ($b -and (Get-Command Get-AssayFromTestsSummaryBundle -ErrorAction SilentlyContinue)) {
                $a = Get-AssayFromTestsSummaryBundle -Bundle $b
                if ($a) { return $a }
            }
        }
    } catch {}

    # Fallback: best effort scan using CsvBundle parser (single source of truth)
    try {
        if (-not (Get-Command Parse-CsvLine -ErrorAction SilentlyContinue)) { return $null }
        if (-not (Get-Command Detect-Delimiter -ErrorAction SilentlyContinue)) { return $null }

        $lines = Get-Content -LiteralPath $Path -TotalCount 250 -ErrorAction Stop
        foreach ($ln in $lines) {
            if ([string]::IsNullOrWhiteSpace($ln)) { continue }
            $delim = Detect-Delimiter -line $ln
...
function Get-CsvStats {
    ...
    # Always prefer bundle (single source of truth for delimiter + indices)
    if (-not $Bundle) {
        try {
            if ($Path -and (Get-Command Get-TestsSummaryBundle -ErrorAction SilentlyContinue)) {
                $Bundle = Get-TestsSummaryBundle -Path $Path
            }
        } catch {
            if (Get-Command Gui-Log -ErrorAction SilentlyContinue) {
                Gui-Log ("⚠️ Get-CsvStats: " + $_.Exception.Message) 'Warn'
            }
            return [pscustomobject]$out
        }
    }

    if (-not $Bundle -or -not $Bundle.Lines) { return [pscustomobject]$out }
    if (-not (Get-Command Parse-CsvLine -ErrorAction SilentlyContinue)) { return [pscustomobject]$out }

    $lines = $Bundle.Lines
    if (-not $lines -or $lines.Count -lt 10) { return [pscustomobject]$out }

    $hdrIdx  = [int]$Bundle.HeaderRowIndex
    $dataIdx = [int]$Bundle.DataStartRowIndex
    $delim   = [string]$Bundle.Delimiter

    if ($hdrIdx -lt 0)  { $hdrIdx = 0 }
    if ($dataIdx -lt 0) { $dataIdx = $hdrIdx + 1 }
    if (-not $delim)    { $delim = ',' }

    # Resolve column indices (normalized header keys)
    $idxCart = -1
    $idxLsp  = -1
    $idxIns  = -1
    try {
        if ($Bundle.HeaderIndex) {
            if ($Bundle.HeaderIndex.ContainsKey('cartridge s/n')) { $idxCart = [int]$Bundle.HeaderIndex['cartridge s/n'] }
            if ($Bundle.HeaderIndex.ContainsKey('reagent lot id')) { $idxLsp  = [int]$Bundle.HeaderIndex['reagent lot id'] }
            if ($Bundle.HeaderIndex.ContainsKey('instrument s/n')) { $idxIns  = [int]$Bundle.HeaderIndex['instrument s/n'] }
        }
    } catch {}

    if ($idxCart -lt 0) { $idxCart = 3 }
    if ($idxLsp  -lt 0) { $idxLsp  = 4 }
    if ($idxIns  -lt 0) { $idxIns  = 6 }
    ...
    for ($r=$dataIdx; $r -lt $lines.Count; $r++) {
        $ln = $lines[$r]
        if (-not $ln -or -not $ln.Trim()) { continue }

        $f = $null
        try { $f = Parse-CsvLine -line $ln -delim $delim } catch { continue }
        if (-not $f) { continue }
    ...
    # Instrument type mapping (GX/Infinity)
    $lut = @{}
...
function Get-InfinitySpFromCsvStrict {
    ...
    if (-not (Get-Command Parse-CsvLine -ErrorAction SilentlyContinue)) { return '—' }
    ...
    # Prefer bundle (single source of truth)
    $bundle = $null
    try {
        if (Get-Command Get-TestsSummaryBundle -ErrorAction SilentlyContinue) {
            $bundle = Get-TestsSummaryBundle -Path $Path
    ...
    $lines = $Lines
    if ($bundle -and $bundle.Lines) { $lines = $bundle.Lines }
    ...
    try {
        if ($bundle) {
            if ($bundle.HeaderRowIndex -ge 0) { $headerIndex = [int]$bundle.HeaderRowIndex }
            if ($bundle.DataStartRowIndex -ge 0) { $dataStart = [int]$bundle.DataStartRowIndex }
            if ($bundle.Delimiter) { $delim = [string]$bundle.Delimiter }
        }
    } catch {}

    $hdr = $null
    try { $hdr = Parse-CsvLine -line $lines[$headerIndex] -delim $delim } catch { $hdr = $null }
    if (-not $hdr) { return '—' }
    ...
    for ($i=0; $i -lt $colCount; $i++) {
        ...
        if ($idxSample -lt 0 -and $h -match 'sample' -and $h -match 'id') { $idxSample = $i }
        if ($idxInstr  -lt 0 -and $h -match 'instrument' -and ($h -match 's/?n' -or $h -match 'serial' -or $h -match 'sn')) { $idxInstr = $i }
        if ($idxSample -ge 0 -and $idxInstr -ge 0) { break }
    }

    if ($idxInstr -lt 0)  { $idxInstr  = 6 }
    if ($idxSample -lt 0) { $idxSample = 2 }
    ...
    for ($r = $dataStart; $r -lt $lines.Count; $r++) {
        ...
        $f = $null
        try { $f = Parse-CsvLine -line $ln -delim $delim } catch { continue }
        if (-not $f -or $f.Count -le [Math]::Max($idxInstr,$idxSample)) { continue }
