...
# --- Tests Summary CSV bundle (read once, reuse everywhere) -------------------
...
function Find-TestsSummaryHeaderRowIndex {
    ...
    for ($i = 0; $i -le $max; $i++) {
        ...
        # Heuristic aligned with RuleEngine: header includes Assay + Test Type
...
function Get-TestsSummaryBundle {
    ...
    $lines = $null
    try { $lines = Get-Content -LiteralPath $Path } catch {
    ...
    $hdrIdx = Find-TestsSummaryHeaderRowIndex -Lines $lines
    if ($hdrIdx -lt 0) { $hdrIdx = 7 } # legacy default
    $dataIdx = Find-TestsSummaryDataStartRowIndex -Lines $lines -HeaderRowIndex $hdrIdx
    if ($dataIdx -lt 0) { $dataIdx = ($hdrIdx + 2) }

    $headers = $null
    try { $headers = ConvertTo-CsvFields $lines[$hdrIdx] } catch { $headers = $null }
    ...
    $bundle = [pscustomobject]@{
        ...
        HeaderRowNumber   = ($hdrIdx + 1)          # 1-based
        DataStartRowIndex = $dataIdx
        DataStartRowNumber= ($dataIdx + 1)         # 1-based
        Headers           = $headers
    }
...
# ---------------------------------------------------------------------------
if (-not (Get-Command Ensure-EPPlus -ErrorAction SilentlyContinue)) {
...
if (-not (Get-Command Load-EPPlus -ErrorAction SilentlyContinue)) {

function Load-EPPlus {
    if ([System.AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { $_.GetName().Name -eq 'EPPlus' }) { return $true }

    try {
        if (Get-Command Initialize-EPPlus -ErrorAction SilentlyContinue) {
            $hint = $null
            try { if ($Config -and $Config.EpplusDllPath) { $hint = $Config.EpplusDllPath } } catch {}
            if (Initialize-EPPlus -HintPath $hint) { return $true }
        }
    } catch {
        Gui-Log "⚠️ EPPlus-loader: $($_.Exception.Message)" 'Warn'
    }
