...
foreach ($r in $results) {
    ...
    if ((($r.Deviation + '')).Trim().ToUpperInvariant() -eq 'ERROR') {
        ...
        $known = $false
        if ($code -and ($code -match '^\d{4,5}$')) {
            try { $known = $errLut.Codes.ContainsKey($code) } catch { $known = $false }
        }
        $isMtbInd = $false
        try {
        ...
        $errName = ''
        if ($known) {
            try { $errName = ($errLut.Codes[$code].Name + '') } catch { $errName = '' }
        }
        if (-not $errName) {
            try { $errName = ($r.ErrorName + '') } catch { $errName = '' }
        }
        $isOther = $false
        try {
            # Kolla efter "Other error codes" (engelska) eller svensk variant "övriga felkoder"
            if ($errName -match '(?i)other\s*error(\s*codes)?' -or $errName -match '(?i)övrig[a]?\s*felkod(er)?') {
                $isOther = $true
            }
        } catch { $isOther = $false }
        if ($isMtbInd) {
...
for ($i = 0; $i -lt $rowCount; $i++) {
    ...
    switch ($rawDev) {
        'ERROR' {
            ...
            # Först hämta namnet på felkoden, antingen från regelbanken eller från raden själv
            $errName = ''
            $known = $false
            if ($errCode -and ($errCode -match '^\d{4,5}$')) {
                try { $known = $errLut.Codes.ContainsKey($errCode) } catch { $known = $false }
                if ($known) {
                    try { $errName = ($errLut.Codes[$errCode].Name + '') } catch { $errName = '' }
                }
            }
            if (-not $errName) {
                try { $errName = ($r.ErrorName + '') } catch { $errName = '' }
            ...
            # Bestäm kategori: MTB indeterminate är alltid Minor
            if ($isMtbInd) {
                $data[$i, 2] = 'Minor Functional'
            }
            elseif ($known -and (-not $isOther)) {
                # Felkod finns i regelbanken och är inte Other error codes -> Minor Functional
                $data[$i, 2] = 'Minor Functional'
            } else {
                # Alla andra -> Instrument Error
