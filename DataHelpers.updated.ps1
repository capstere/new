...
# Ensure the rule engine (pure helpers around RuleBank) is available when this module is dot-sourced.
if (-not (Get-Command Get-RuleBankRoot -ErrorAction SilentlyContinue)) {
    $ruleEnginePath = Join-Path $PSScriptRoot 'RuleEngine.ps1'
    if (Test-Path -LiteralPath $ruleEnginePath) {
        . $ruleEnginePath
    }
}
...
function Invoke-AssayReportBuild {
    ...
    try {
        ...
        $ruleEngineRows = @()
        try {
            if ($csvLines -and $csvLines.Count -gt 9) {
                $ruleHeader = ConvertTo-CsvFields $csvLines[7]
                $ruleDataLines = $csvLines[9..($csvLines.Count - 1)] | Where-Object { $_ -and $_.Trim() }
                if ($ruleHeader -and $ruleDataLines.Count -gt 0) {
                    $ruleEngineRows = ConvertFrom-Csv -InputObject ($ruleDataLines -join "`n") -Header $ruleHeader
                }
            }
        } catch {
            Gui-Log ("⚠️ RuleEngine: kunde inte läsa dataset från CSV: " + $_.Exception.Message) 'Warn'
        }

        try {
            if (Get-Command New-AssayRuleContext -ErrorAction SilentlyContinue) {
                $assayRuleContext = New-AssayRuleContext -Rows $ruleEngineRows -FallbackAssayName $runAssay
            }
        } catch {
            Gui-Log ("⚠️ RuleEngine: kunde inte bygga assaykontext: " + $_.Exception.Message) 'Warn'
        }

        if (-not $assayRuleContext) {
            $fallbackFindings = New-Object 'System.Collections.Generic.List[string]'
            $fallbackFindings.Add('RuleBank saknas – defaultprofil används.')
            $assayRuleContext = [pscustomobject]@{
                AssayName        = $runAssay
                AssayVersion     = $null
                ReagentLotId     = $null
                ProfileKey       = 'default'
                Mode             = 'GlobalOnly'
                Profile          = $null
                IdentityFindings = $fallbackFindings
                VersionMatch     = $null
            }
        }

        $assayIdentitySummary = 'Inga avvikelser'
        try {
            if ($assayRuleContext.IdentityFindings -and $assayRuleContext.IdentityFindings.Count -gt 0) {
                $assayIdentitySummary = ($assayRuleContext.IdentityFindings | Where-Object { $_ }) -join '; '
            }
        } catch { $assayIdentitySummary = 'N/A' }

        $logAssayName    = if ($assayRuleContext.AssayName)    { $assayRuleContext.AssayName }    else { 'N/A' }
        $logAssayVersion = if ($assayRuleContext.AssayVersion) { $assayRuleContext.AssayVersion } else { 'N/A' }
        $logProfileKey   = if ($assayRuleContext.ProfileKey)   { $assayRuleContext.ProfileKey }   else { 'default' }
        $logMode         = if ($assayRuleContext.Mode)         { $assayRuleContext.Mode }         else { 'GlobalOnly' }
        $versionMatchText = 'N/A'
        if ($null -ne $assayRuleContext.VersionMatch) { $versionMatchText = if ($assayRuleContext.VersionMatch) { 'True' } else { 'False' } }
        Gui-Log ("RuleBank: using profile '{0}' (mode: {1}) for assay '{2}' v{3}. Version match: {4}." -f `
                $logProfileKey, $logMode, $logAssayName, $logAssayVersion, $versionMatchText) 'Info'
    ...
    # === RuleBank summary     ===
    # ============================
    # Varför: Detta gör det spårbart vilket regelsystem/profil som användes för att bygga rapporten,
    # utan att ändra hur användaren jobbar. Viktigt för QA/audit när data ser "konstig" ut.
    try {
        $rb = $null
        try { $rb = Get-RuleBankRoot } catch { $rb = $null }
        $rbVersion = $null
        if ($rb -and $rb.ContainsKey('Version')) { $rbVersion = ($rb.Version + '').Trim() }
        if (-not $rbVersion) { $rbVersion = 'N/A' }

        $rbAssay   = if ($assayRuleContext -and $assayRuleContext.AssayName)    { $assayRuleContext.AssayName }    else { if ($runAssay) { $runAssay } else { 'N/A' } }
        $rbAssayV  = if ($assayRuleContext -and $assayRuleContext.AssayVersion) { $assayRuleContext.AssayVersion } else { 'N/A' }
        $rbProfile = if ($assayRuleContext -and $assayRuleContext.ProfileKey)   { $assayRuleContext.ProfileKey }   else { 'default' }
        $rbMode    = if ($assayRuleContext -and $assayRuleContext.Mode)         { $assayRuleContext.Mode }         else { 'GlobalOnly' }

        $rbFindings = 'Inga avvikelser'
        try {
            if ($assayRuleContext -and $assayRuleContext.IdentityFindings -and $assayRuleContext.IdentityFindings.Count -gt 0) {
                $rbFindings = ($assayRuleContext.IdentityFindings | Where-Object { $_ }) -join '; '
            }
        } catch { $rbFindings = 'N/A' }

        # Lägg under mallens sista block (row 41+) för att aldrig störa befintliga fält.
        $wsInfo.Cells['A41'].Value = 'RuleBank Summary'
        $wsInfo.Cells['A41'].Style.Font.Bold = $true

        $wsInfo.Cells['A42'].Value = 'RuleBank Version'
        $wsInfo.Cells['B42'].Style.Numberformat.Format = '@'
        $wsInfo.Cells['B42'].Value = $rbVersion

        $wsInfo.Cells['A43'].Value = 'Assay'
        $wsInfo.Cells['B43'].Style.Numberformat.Format = '@'
        $wsInfo.Cells['B43'].Value = $rbAssay

        $wsInfo.Cells['A44'].Value = 'Assay Version'
        $wsInfo.Cells['B44'].Style.Numberformat.Format = '@'
        $wsInfo.Cells['B44'].Value = $rbAssayV

        $wsInfo.Cells['A45'].Value = 'Profile'
        $wsInfo.Cells['B45'].Style.Numberformat.Format = '@'
        $wsInfo.Cells['B45'].Value = $rbProfile

        $wsInfo.Cells['A46'].Value = 'Mode'
        $wsInfo.Cells['B46'].Style.Numberformat.Format = '@'
        $wsInfo.Cells['B46'].Value = $rbMode

        $wsInfo.Cells['A47'].Value = 'IdentityFindings'
        $wsInfo.Cells['B47'].Style.WrapText = $true
        $wsInfo.Cells['B47'].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
        $wsInfo.Cells['B47'].Style.Numberformat.Format = '@'
        $wsInfo.Cells['B47'].Value = $rbFindings
        try { $wsInfo.Row(47).CustomHeight = $true; $wsInfo.Row(47).Height = 45 } catch {}
        try { $wsInfo.Column(2).Width = 65 } catch {}
    } catch {
        Gui-Log ("⚠️ RuleBank summary: " + $_.Exception.Message) 'Warn'
    }
    ...
    Gui-Log ("⚠️ Kunde inte extrahera utrustning från Test Summary: " + $_.Exception.Message) 'Warn'
            ...
            # --- RuleBank / Assay context summary (placed below existing template rows to avoid layout conflicts) ---
            try {
                $ruleBankVersion = $null
                try {
                    $rbMeta = Get-RuleBankRoot
                    if ($rbMeta -and $rbMeta.Version) { $ruleBankVersion = $rbMeta.Version }
                } catch {}
                ...
                $valueColumn = 'B'
                $ruleSummary = @(
                    @{ Label = 'RuleBank Version';   Value = if ($ruleBankVersion) { $ruleBankVersion } else { 'N/A' } }
                    @{ Label = 'Assay (RuleBank)';   Value = if ($assayRuleContext.AssayName) { $assayRuleContext.AssayName } else { 'N/A' } }
                    @{ Label = 'Assay Version';      Value = if ($assayRuleContext.AssayVersion) { $assayRuleContext.AssayVersion } else { 'N/A' } }
                    @{ Label = 'Rule Profile';       Value = if ($assayRuleContext.ProfileKey) { $assayRuleContext.ProfileKey } else { 'default' } }
                    @{ Label = 'Rule Mode';          Value = if ($assayRuleContext.Mode) { $assayRuleContext.Mode } else { 'GlobalOnly' } }
                    @{ Label = 'Identity Findings';  Value = $assayIdentitySummary }
                )
                $rowIdxRule = $ruleInfoStartRow
                foreach ($rowInfo in $ruleSummary) {
                    $wsInfo.Cells["$labelColumn$rowIdxRule"].Value = $rowInfo.Label
                    $wsInfo.Cells["$labelColumn$rowIdxRule"].Style.Font.Bold = $true
                    $wsInfo.Cells["$valueColumn$rowIdxRule"].Style.Numberformat.Format = '@'
                    $wsInfo.Cells["$valueColumn$rowIdxRule"].Value = $rowInfo.Value
                    $rowIdxRule++
                }
            } catch {
                Gui-Log ("⚠️ RuleBank-info kunde inte skrivas: " + $_.Exception.Message) 'Warn'
            }
        } catch {
    ...
    Gui-Log "⚠️ Information-blad fel: $($_.Exception.Message)" 'Warn'
        ...
        # POLICY: Rapporten sparas alltid temporärt (TEMP) och öppnas.
        # Varför: minskar risken att någon av misstag sparar en "UNCONTROLLED" rapport in i LSP-mappen
        # och förenklar det regulatoriska arbetsflödet.
        $SaveInLsp = $false
        $saveDir = $env:TEMP
