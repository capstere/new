...
foreach ($r in $results) {
    ...
    if ((($r.Deviation + '')).Trim().ToUpperInvariant() -eq 'ERROR') {
        ...
        $isKnown = $false
        try { $isKnown = $errLut.Codes.ContainsKey($code) } catch { $isKnown = $false }
        $isMtbInd = $false
        try {
...
for ($i = 0; $i -lt $rowCount; $i++) {
    ...
    switch ($rawDev) {
        'ERROR' {
            # Klassificera fel som Minor eller Instrument baserat på om felkoden finns i regelbanken.
            # Felkoder som finns i regelbanken (oavsett längd) betraktas som Minor Functional. Alla övriga (ej funna) blir Instrument Error.
            $isKnown = $false
            try { $isKnown = $errLut.Codes.ContainsKey($errCode) } catch { $isKnown = $false }
            if ($isMtbInd) {
