...
$btnScan.Add_Click({
    ...
    foreach ($path in $RootPaths) {
        try {
            # Snabb väg: kontrollera direkta barn först (vanligast)
            $folder = Get-ChildItem -LiteralPath $path -Directory -ErrorAction SilentlyContinue |
                      Where-Object { $_.Name -match "#?$lsp" } |
                      Sort-Object LastWriteTime -Descending |
                      Select-Object -First 1
            if ($folder) { break }

            # Fallback: rekursivt (om LSP-mappen ligger djupare)
            $folder = Get-ChildItem -LiteralPath $path -Directory -Recurse -ErrorAction SilentlyContinue |
                      Where-Object { $_.Name -match "#?$lsp" } |
                      Sort-Object LastWriteTime -Descending |
                      Select-Object -First 1
            if ($folder) { break }
        ...
        if ($isKV) {
            ...
            try {
                if ($ws.Dimension) {
                    ...
                    $ws.Cells[1,1,$maxR,2].AutoFitColumns() | Out-Null
                }
            } catch {}
        }
        return $true
...
#region REGION: Formatting
        ...
        if ($totalRows -eq 0) {
            ...
            if ($wsOut2.Dimension) { try { $maxR = [Math]::Min($wsOut2.Dimension.End.Row, (1 + $totalRows + 1)); $wsOut2.Cells[1,1,$maxR,8].AutoFitColumns() | Out-Null } catch {} }
        }
...
#region REGION: Worksheet
    ...
    try {
        if (-not $selLsp) {
            # Försök först använda listan från 'Sök filer' (ingen ny disk-sökning)
            if ($clbLsp -and $clbLsp.Items.Count -gt 0) {
                try {
                    $best = @($clbLsp.Items) | Sort-Object LastWriteTime -Descending | Select-Object -First 1
                    if ($best) { $selLsp = ([System.IO.FileInfo]$best).FullName }
                } catch {}
            }
    ...
    Gui-Log ("⚠️ ⚠️ Information2 (Rules) fel: {0}`n{1}`n{2}" -f $msg, $pos, $stk) 'Warn'
        ...
        try {
            if ($controlTab -and (Test-Path -LiteralPath $RawDataPath)) {
                ...
                # (Perf) Undvik Workbook.Calculate() här – kan vara extremt långsamt på stora kontrollfiler
$candidates = if ($controlTab -match '\|') { $controlTab -split '\|' | ForEach-Object { $_.Trim() } | Where-Object { $_ } } else { @($controlTab) }
                ...
                if ($srcWs) {
                    ...
                    if ($wsCM.Dimension) {
                        ...
                        try { if ($wsCM.Dimension) { $maxR = [Math]::Min($wsCM.Dimension.End.Row, 300); $maxC = [Math]::Min($wsCM.Dimension.End.Column, 20); $wsCM.Cells[1,1,$maxR,$maxC].AutoFitColumns() | Out-Null } } catch {}
                    }
            ...
            if ($chkSharePointInfo -and -not $chkSharePointInfo.Checked) {
                ...
                if (-not $batch) {
                    ...
                    if (Get-Command Write-SPSheet-Safe -ErrorAction SilentlyContinue) {
                        ...
                        try { if ($wsSp.Dimension) { $maxR = [Math]::Min($wsSp.Dimension.End.Row, 250); $maxC = [Math]::Min($wsSp.Dimension.End.Column, 16); $wsSp.Cells[1,1,$maxR,$maxC].AutoFitColumns() | Out-Null } } catch {}
                    }
                } else {
